...
class PHPExcel_Writer_HTML extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter {
	...
	public function save_excel_to_html($pFilename = null) {
		// garbage collect
		$this->_phpExcel->garbageCollect();
		...
		PHPExcel_Calculation::setArrayReturnType(PHPExcel_Calculation::RETURN_ARRAY_AS_VALUE);
               ...
               // return $this->generateHTMLHeader(!$this->_useInlineCss).$this->generateSheetData().$this->generateHTMLFooter();
		 
                
                return $this->generateSheetData();
		// Build CSS
		$this->buildCSS(!$this->_useInlineCss);
	...
	public function save_excel_to_html_underlying($pFilename = null) {
		// garbage collect
		$this->_phpExcel->garbageCollect();
		...
		PHPExcel_Calculation::setArrayReturnType(PHPExcel_Calculation::RETURN_ARRAY_AS_VALUE);
                ...
                return $this->generateSheetData_underlying();
        ...
        public function save_excel_to_html_term_fund_information($pFilename = null) {
		// garbage collect
		$this->_phpExcel->garbageCollect();
		...
		PHPExcel_Calculation::setArrayReturnType(PHPExcel_Calculation::RETURN_ARRAY_AS_VALUE);
                ...
                return $this->generateSheetData_term_fund_information();
	...
	public function save_excel_to_html_term($pFilename = null) {
		// garbage collect
		$this->_phpExcel->garbageCollect();
		...
		PHPExcel_Calculation::setArrayReturnType(PHPExcel_Calculation::RETURN_ARRAY_AS_VALUE);
                ...
                return $this->generateSheetData_b();
        ...
        //fund_recommendation
	public function save_excel_to_html_fr($pFilename = null) {
		// garbage collect
		$this->_phpExcel->garbageCollect();
		...
		PHPExcel_Calculation::setArrayReturnType(PHPExcel_Calculation::RETURN_ARRAY_AS_VALUE);
                ...
                return $this->generateSheetData_fr();
	...
	//fund_recommendation
	public function save_excel_to_html_b($pFilename = null) {
		// garbage collect
		$this->_phpExcel->garbageCollect();
	...
	public function generateSheetData_underlying() {
		// PHPExcel object known?
		if (is_null($this->_phpExcel)) {
			throw new PHPExcel_Writer_Exception('Internal PHPExcel object not set to an instance of an object.');
		}
		...
		foreach ($sheets as $sheet) {
			// Write table header
			$html .= $this->_generateTableHeader_u($sheet);
			...
			$dimension[0][0] = PHPExcel_Cell::columnIndexFromString($dimension[0][0]) - 1;
			$dimension[1] = PHPExcel_Cell::coordinateFromString($dimension[1]);
			$dimension[1][0] = 11;//PHPExcel_Cell::columnIndexFromString($dimension[1][0]) - 1;
			...
			while($row++ < $rowMax) {
				...
				if ( !isset($this->_isSpannedRow[$sheet->getParent()->getIndex($sheet)][$row]) ) {
					...
					$html .= $this->_generateRow_u($sheet, $rowData, $row - 1);
				}
        ...
        public function generateSheetData_term_fund_information() {
		// PHPExcel object known?
		if (is_null($this->_phpExcel)) {
			throw new PHPExcel_Writer_Exception('Internal PHPExcel object not set to an instance of an object.');
		}
		...
		foreach ($sheets as $sheet) {
			...
			$dimension[0][0] = PHPExcel_Cell::columnIndexFromString($dimension[0][0]) - 1;
			$dimension[1] = PHPExcel_Cell::coordinateFromString($dimension[1]);
			$dimension[1][0] = 6;//PHPExcel_Cell::columnIndexFromString($dimension[1][0]) - 1;
	...
	public function generateSheetData_fr() {
		// PHPExcel object known?
		if (is_null($this->_phpExcel)) {
			throw new PHPExcel_Writer_Exception('Internal PHPExcel object not set to an instance of an object.');
		}
		...
		foreach ($sheets as $sheet) {
			// Write table header
			$html .= $this->_generateTableHeader_fr($sheet);
			...
			$dimension[0][0] = PHPExcel_Cell::columnIndexFromString($dimension[0][0]) - 1;
			$dimension[1] = PHPExcel_Cell::coordinateFromString($dimension[1]);
			$dimension[1][0] = 3;//PHPExcel_Cell::columnIndexFromString($dimension[1][0]) - 1;

			// row min,max
			$rowMin = 5;//$dimension[0][1];
			$rowMax = $dimension[1][1];
			...
			while($row++ < $rowMax) {
				...
				if ( !isset($this->_isSpannedRow[$sheet->getParent()->getIndex($sheet)][$row]) ) {
					...
					$html .= $this->_generateRow_fr($sheet, $rowData, $row - 1);
				}
	...
	public function generateSheetData_b() {
		// PHPExcel object known?
		if (is_null($this->_phpExcel)) {
			throw new PHPExcel_Writer_Exception('Internal PHPExcel object not set to an instance of an object.');
		}
		...
		foreach ($sheets as $sheet) {
			...
			$dimension[0][0] = PHPExcel_Cell::columnIndexFromString($dimension[0][0]) - 1;
			$dimension[1] = PHPExcel_Cell::coordinateFromString($dimension[1]);
			$dimension[1][0] = 26;//PHPExcel_Cell::columnIndexFromString($dimension[1][0]) - 1;

			// row min,max
			$rowMin = 3;//$dimension[0][1];
			$rowMax = $dimension[1][1];
...
//		$css = $this->_mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
	...
	private function _generateTableHeader_u($pSheet) {
		$sheetIndex = $pSheet->getParent()->getIndex($pSheet);
		...
		if (!$this->_useInlineCss) {
			$gridlines = $pSheet->getShowGridlines() ? ' gridlines' : '';
			$html .= '	<table border="0" cellpadding="0" cellspacing="0" id="sheet' . $sheetIndex . '"  class="table table-bordered table-striped">' . PHP_EOL;
		} else {
			

			if ($this->_isPdf && $pSheet->getShowGridlines()) {
				$html .= '	<table class="table table-bordered table-striped" border="1" cellpadding="1" id="sheet' . $sheetIndex . '" cellspacing="1" >' . PHP_EOL;
			} else {
				$html .= '	<table class="table table-bordered table-striped" border="0" cellpadding="1" id="sheet' . $sheetIndex . '" cellspacing="0" >' . PHP_EOL;
			}
		}
	...
	private function _generateTableHeader_fr($pSheet) {
		$sheetIndex = $pSheet->getParent()->getIndex($pSheet);
	...
	private function _generateRow_u(PHPExcel_Worksheet $pSheet, $pValues = null, $pRow = 0) {
		if (is_array($pValues)) {
			// Construct HTML
			$html = '';
			...
			if ($this->_isPdf && count($pSheet->getBreaks()) > 0) {
				...
				//	$html .= '<div style="page-break-before:always" />';
					...
					// open table again: <table> + <col> etc.
				//	$html .= $this->_generateTableHeader_u($pSheet);
				}
			}
	...
	private function _generateRow_fr(PHPExcel_Worksheet $pSheet, $pValues = null, $pRow = 0) {
		if (is_array($pValues)) {
			// Construct HTML
			$html = '';
			...
			if ($this->_isPdf && count($pSheet->getBreaks()) > 0) {
				...
				//	$html .= '<div style="page-break-before:always" />';
					...
					// open table again: <table> + <col> etc.
				//	$html .= $this->_generateTableHeader_fr($pSheet);
				}
			}
